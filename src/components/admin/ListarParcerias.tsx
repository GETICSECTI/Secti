...
export const ListarParcerias = ({
  ...
  emptyStateDescription = 'Não há parcerias cadastradas no momento.',
  showHeader = true,
  downloadFn = downloadParceria,
}: ListarParceriasProps) => {
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [selectedParceria, setSelectedParceria] = useState<Parceria | null>(null);
  const [downloadingId, setDownloadingId] = useState<number | null>(null);
  const [downloadError, setDownloadError] = useState<string | null>(null);

  const handleDownload = async (parceria: Parceria) => {
    if (!parceria.caminhoArquivo) return;
  ...
  const getIconForType = (tipo: Parceria['tipo']) => {
    switch (tipo) {
      case 'pdf':
        return (
  ...
  const handleDelete = (id: number, nome: string) => {
    setSelectedParceria({ id, nome } as Parceria);
    setDeleteModalOpen(true);
  };

  const confirmDelete = () => {
    if (selectedParceria) {
      onDelete(selectedParceria.id);
  ...
  return (
    <>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {parcerias.map((parceria) => (
              <tr key={parceria.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="shrink-0">{getIconForType(parceria.tipo)}</div>
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-900">{parceria.nome}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-xs font-medium text-gray-500 uppercase">{parceria.tipo}</span>
                </td>
      ...
      <DeleteModal
        ...
        message={`Tem certeza de que deseja excluir a parceria "${selectedParceria?.nome}"? Esta ação não pode ser desfeita.`}
        confirmText="Excluir"
        cancelText="Cancelar"
      />
    </>
