...
export const ListarProcessos = ({
  ...
  emptyStateDescription = 'Não há processos cadastrados no momento.',
  showHeader = true,
  downloadFn = downloadProcesso,
}: ListarProcessosProps) => {
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [selectedProcesso, setSelectedProcesso] = useState<Processo | null>(null);
  const [downloadingId, setDownloadingId] = useState<number | null>(null);
  const [downloadError, setDownloadError] = useState<string | null>(null);

  const handleDownload = async (processo: Processo) => {
    if (!processo.caminhoArquivo) return;
  ...
  const getIconForType = (tipo: Processo['tipo']) => {
    switch (tipo) {
      case 'pdf':
        return (
  ...
  const handleDelete = (id: number, nome: string) => {
    setSelectedProcesso({ id, nome } as Processo);
  ...
  return (
    <>
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
        {showHeader && (
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-lg font-medium text-gray-900">Processos Cadastrados</h3>
          </div>
        )}
                ...
                <tr key={processo.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="shrink-0">{getIconForType(processo.tipo)}</div>
                      <div className="ml-4">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">
                      {processo.tags && processo.tags.length > 0
                        ? processo.tags.map((tag) => tag.nome).join(', ')
                        : 'Sem tags'}
                    </div>
                  </td>
            ...
            <div key={processo.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  {getIconForType(processo.tipo)}
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 line-clamp-2">{processo.nome}</p>
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                      <span className="uppercase font-medium">{processo.tipo}</span>
                      <span>•</span>
                      <span>Publicado em: {formatarDataBrasileira(processo.dataPublicacao)}</span>
                      {processo.tags && processo.tags.length > 0 && (
                        <>
                          <span>•</span>
                          <span>Tags: {processo.tags.map((tag) => tag.nome).join(', ')}</span>
                        </>
                      )}
                    </div>
                  </div>
      ...
      <DeleteModal
        ...
        message={`Tem certeza de que deseja excluir o processo "${selectedProcesso?.nome}"? Esta ação não pode ser desfeita.`}
        confirmText="Excluir"
        cancelText="Cancelar"
      />
    </>
