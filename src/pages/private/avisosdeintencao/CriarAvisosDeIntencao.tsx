...
import { avisosIntencaoContratarService } from '../../../services/avisosIntencaoContratarService';
import { TagSelector } from '../../../components/admin/TagSelector';

export const CriarAvisosDeIntencao = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.tagIds.length === 0) {
      setErro('Por favor, selecione uma tag');
      return;
    }
    ...
    try {
      await avisosIntencaoContratarService.cadastrar({
      ...
      setSucesso('Aviso de intenção criado com sucesso!');

      // Aguardar um momento para mostrar a mensagem de sucesso
      setTimeout(() => {
        navigate('/admin/avisosdeintencao');
      }, 1500);
    } catch (error) {
      const mensagemErro = error instanceof Error ? error.message : 'Erro ao criar aviso de intenção';
    ...
    <PrivateLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Novo Aviso de Intenção</h1>
            <p className="text-gray-600 mt-2">Preencha os campos abaixo para criar um novo aviso de intenção</p>
          </div>
          <button
            type="button"
            onClick={() => navigate('/admin/avisosdeintencao')}
        ...
        {erro && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-medium text-red-800">Erro ao criar aviso</h3>
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0C2856] focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">Selecione a data de publicação do aviso</p>
          </div>
          ...
          <div>
            <label htmlFor="arquivo-input" className="block text-sm font-medium text-gray-700 mb-2">
              Selecione o Arquivo do Aviso de Intenção <span className="text-red-500">*</span>
            </label>
